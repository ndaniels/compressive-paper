...
as FragBag, a greedy k-centers algorithm, and vector difference to generate a
the clustered representation.
...
Then in the first pass, proteins were selected as cluster centers if and only if they were not within a user-specified distance $r_c$ from an existing center.
In the second pass, every protein was assigned to the nearest cluster center.

\subsection*{Search}
Similarity search here is exactly as described in the data structure section previous.
For a given search query $q$ and search radius $r$,
\begin{itemize}
    \item A coarse search was performed to find all cluster centers within distance $r+r_c$ of $q$.
