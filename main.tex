...
Furthermore, the entropy of the distribution over databases constrained by this tree is bounded by the cluster radii to be $O((n-k)r_c)$.
Thus, given our metric entropy bound, we can encode the entire database in $O(kl + (n-k)(r_c + \log S_c))$ bits.
