...
In many fields, and in particular molecular biology, this increase is exponential in nature, even outpacing Moore’s and Kryder’s laws on the respective doublings of transistors on a chip and long-term data storage density [cite].
...
Although this problem is difficult in general for high-dimensional spaces, real-world instantiations, especially in molecular biology, exhibit additional structure we can exploit.

Specifically, if similarity is defined by some metric-like distance function and the database exhibits both low `entropy' and low `fractal dimension', we can do much better than na\"ive methods, with similarity search times proportional to the entropy of the database.
We will define `entropy' and `fractal dimension' more precisely later, but intuitively, they are respectively measures of the total information content and scaling behavior of points in spheres as the radius changes.
In this paper, we introduce a data structure designed to 
