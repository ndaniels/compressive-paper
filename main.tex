...
sometimes different distance functions can be used for the clustering vs the searches;
and sometimes even what counts as a distinct point is not entirely clear without domain knowledge.
%
...
These applications are enabled by augmenting the data structure with the usage of different distance functions in different stages of the algorithm and preprocessing to take advantage of domain specific knowledge.
...
In developing CaBLASTX, we augmented the entropy scaling data structure by using
different distance functions for clustering vs search.
...
As shown in Table~\ref{mgbench}, on the metagenomic assemblies, where query-side 
...
Using an entropy-scaling approach, our new software Ammolite performs MCS based search in time that scales with the entropy of a database.\hl{?????}.
...
tool that provides ~4x speed-up with greater than 95\% accuracy compared to a gold standard.

MCS based search of molecule databases typically matches pairs of molecules by their overlap coefficients. The overlap coefficient of two molecules is not a metric.
Molecules with a high overlap coefficient may be globally dissimilar if the larger molecule contains a signifigant subgraph of the smaller molecule. This is useful 
for identifying relevant structures in a molecule database but means that an approach to entropy-scaling based on a metric like graph distance \cite{bunke1998graph}
may not be particularly useful. Since finding the MCS of two molecules is an extremely costly operation we instead chose to base our approach to entropy-scaling
off of molecular substructures. 
