...
For ease of analysis, we will work in a high-dimensional metric space and consider the database as a set of $n$ unique points in that metric space.
...
$k$ cluster centers are chosen such that no cluster has radius greater than a user-specified parameter $r_c$.
...
The coarse search is over the list of cluster centers, returning all clusters centers within distance $r + r_c$ of $q$.
For the fine search, each cluster returned from the coarse search is expanded and those items are compared against $q$ to find all items within radius $r$.
...
As mentioned, we want to ensure that $d$ is a small constant.
...
Then by the triangle inequality, only clusters with centers within distance $r+r_c$ from $q$ contain entries within distance $r$ of $q$.
Because we assumed a small constant fractal dimension $d$, in expectation, only a constant number of clusters will need to be checked.
Thus, non-cluster centers will only have to be checked $\frac{C}{k}$ proportion of the time, for some constant $C$.
