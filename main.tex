...
Recall that by the triangle inequality, only the clusters with centers within distance $r+r_c$ from $q$ contain entries within distance $r$ of $q$.
Additionally by the triangle inequality, the union of those clusters $F \subset B_D(q,r+r_c)$.
We can then bound $|F| \le |B_D(q,r+r_c)|$.
...
in expectation, only a constant number of clusters will need to be checked.
Thus, non-cluster centers will only have to be checked $\frac{C}{k}$ proportion of the time, for $C = \left(\frac{r+r_c}{r}\right)^d$.
