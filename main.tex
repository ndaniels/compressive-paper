...
smarter analysis tools.
    ...
    Ammolite, 150x speedup of small molecule similarity search with less than 7\% loss in sensitivity; CaBLASTX, 654x speedup of BLASTX with less than 5\% loss in sensitivity; and esFragBag, 10x speedup of FragBag with less than 0.2\% loss in sensitivity.
...
tool that provides approximately a factor of 150 speed-up with greater than 93\% accuracy compared to the popular SMSD~\cite{rahman2009small}.
...
CaBLASTX clustering of the September 17, 2014 NCBI NR database required 39 
...
the American Gut Microbiome project, we found that caBLASTX provides 
...
while caBLASTX took an average of 89 minutes, a speedup of 654x.
We note that the time spent performing query-side clustering is included for caBLASTX here; without query-side clustering, caBLASTX takes 2,278 minutes, a speedup of 25x.
...
We benchmarked caBLASTX versus BLASTX on a three sets of assembled sequences from the Human Gut Microbiome project (assemblies SRS013215, SRS048536, SRS078197) consisting of 49,041 assembled sequences
from human gut microbiota, ~40 megabases in total, searching against the NCBI's
`NR' non-redundant protein database from September 2014.
The average running time of BLASTX was 4190 minutes, compared to 158 minutes for 
caBLASTX, a speedup of 26.5.
...
\hline
dataset & BLASTX & caBLASTX$^1$ & RapSearch2 & Diamond & caBLASTX$^2$ \\
\hline
NGS reads$^3$ & 58215 (1561.8) & 89 (7.0) & 206 (5.4) & 54 (1.1) & 2278 (102.7) \\
\hline
assemblies$^4$ & 4190 (2105.3) & N/A & 242 (108.0) & 63.7 (26.5) & 158 (75.7) \\
\hline
\end{tabular}\\{
Running time of metagenomic search\\
All values are in minutes, with standard deviation in parentheses.\\
(1) caBLASTX with query-side clustering\\
(2) caBLASTX without query-side clustering\\
(3) American gut microbiome project read sets ERR335622, ERR335625, ERR335631, ERR335635, ERR335636\\
(4) Human gut microbiome project assemblies SRS013215, SRS048536, SRS078197
}
\end{subtable}
...
\begin{tabular}{lllll}
\hline
dataset & caBLASTX$^1$ & RapSearch2 & Diamond & caBLASTX$^2$ \\
\hline
NGS reads$^3$ & 96.1\% (1.0) & 79.5\% (1.63) & 76.6\% (1.05) & 98.9\% (0.54) \\
\hline
assemblies$^4$ & N/A & 88.7\% (1.4) & 92.5\% (1.05) & 99.1\% (0.65)\\
\hline
\end{tabular}\\{
Percentage accuracy compared to BLASTX (which is 100\%)\\
...
than other methods: 99.1\% accuracy on assemblies and 96.1\% 
...
The cluster generation required anywhere from 65 to 23,714 seconds, depending on the choice of radii (See table \ref{tab:esfragbag_clustering}) and no more than a small constant (<3) times as much memory as it takes to simply load the PDB database.
\begin{table}
...
A hierarchical clustering approach, rather than our flat clustering, might 
produce further gains.
The distance function used need not be the same for all levels of the 
hierarchy; indeed, both the alphabet reduction employed in caBLASTX and the 
molecule size heuristic in Ammolite can be thought of as initial versions of a multi-level or hierarchical clustering.
...
This, too, can be thought of as an additional level of clustering.
...
By novelly borrowing the concepts of metric entropy and fractal dimension for our analysis, we are able to give an easier to understand and more intuitive, if somewhat looser, bound on entropy-scaling search tree complexity.
...
a new data set.
First, given a new data set, we must define what the high-dimensional space is.
...
Given the high dimensional space, we must determine how database entries map
onto points (for example, in the case of caBLASTX, they are projected onto a 
reduced alphabet, and are greedily broken into subsequences with a minimum 
length).
Next, clustering must be implemented; a simple greedy clustering may suffice 
(as for esFragBag) but clustering of sequence data may be dramatically 
accelerated by using BLAST-style seed-and-extend matching (as used in caBLASTX).
Finally, coarse and fine search must be implemented; in many cases, existing
tools may be used ``out of the box,'' as with esFragBag and caBLASTX.
With caBLASTX, we note that coarse search---due to the alphabet reduction---used
BLASTN rather than BLASTX, but it was still a standard tool.
