...
For any similarity search query looking for all points within distance $r$ of a query, by the triangle inequality, we need only look in nearby spheres with centers within a distance $r+r_c$ of the query, which by the Triangle Inequality only contain points within distance $r+2r_c$ of the query.
%ref Figure 1?.
...
Thus, doubling a search radius (or increasing from $r$ to $r+2r_c$) only doubles (or increases by a factor of $\frac{r+2r_c}{r}$) the number of points that need to be searched in a fine search after spheres have been identied.
