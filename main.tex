...
providing a useful tool for metagenomic research, which can be readily plugged in to existing analysis pipelines.
...
BLASTX as well as the RapSearch2~\cite{zhao2012rapsearch2} and the recently-released
Diamond~\cite{buchfink2014fast} on a large-scale publicly-available database (Supplemental Methods), and found substantial runtime improvements 
...
expense of accuracy, particularly on raw reads.
% BAB: how about space?
...
Nevertheless, overlap coefficient is typically more desirable for identifying relevant structures in a 
...
The clustering framework extends the approach of caBLASTP~\cite{daniels2013compressive}.
