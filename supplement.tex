...
Additionally, updates can be performed in $O(k)$ time.
...
Note that with updates of this kind, items are no longer guaranteed to be assigned to the nearest cluster center; however, they are still guaranteed to be assigned to some cluster center within distance $r_c$, which is all that is needed for entropy-scaling to work.
