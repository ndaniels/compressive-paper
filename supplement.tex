...
unlike entropy-scaling data structures though, compressed data structures do not measure time-complexity in terms of metric entropy.
...
Next, we relate the space-complexity for the data structure to information-theoretic entropy.
...
Naturally, information-theoretic entropy is a measure of the uncertain of a distribution or random variable, and is not well-defined for a database.
...
Obviously, even with linear decompression, decompressing the entire database for each similarity search would render the preceding section moot.
...
This results from the possibility that redundancy across clusters that would originally have been exploited by the compressor can no longer be once the database is partitioned.
Intuitively, for any fixed-window or block compressor, grouping together similar items into clusters, should increase the performance of the compressor, but it is unclear \textit{a priori} if that balances out the loss of redudancy across clusters.

A somewhat more sophisticated solution is to reorder the entries of the database by cluster, compress the entire database, but then store indexed offsets into the start locations for each cluster.
