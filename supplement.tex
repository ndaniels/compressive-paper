...
In this paper, we make use of three different notions of fractal dimension, including the more intuitive formulation in the main paper, a more classical definition in this following section, and one from which our time-complexity bound directly follows.
In order to both unify these different notions of fractal dimension and to allow us to prove our complexity bounds, we make two technical assumptions about the self-similarity of the data set.
Roughly speaking, we require that the clusters look similar to each other for the cluster radii of interest, in terms of sizes.

For convenience of proof, the two technical assumptions will be that
\begin{itemize}
    \item For a given cluster radius, there is a choice of cluster centers such that all clusters have about the same number of points (which we will call cluster weight), up to a constant multiplicative factor of one another.
    \item For a given radius, the distribution of the number of neighbors within that radius around any point has standard deviation less than a constant multiple of the mean number of neighbors.
\end{itemize}
Of course, these two conditions are closely related, but making each explicitly will make unifying our different notions of fractal dimension more straight-forward.

Additionally, although these two particular formulations of cluster similarity may not be present in all data sets of interest, if the data set does not have some self-similarity, then the notion of fractal dimension start breaking down because the data set is no longer fractal like.
It is far less convenient to try to define fractal dimension when fractal dimension varies widely in different parts of the space.
...
This definition of fractal dimension is very classical, but the reader will note very different in formulation from the intuitive one given in the main text.
However, the two notions are related given the bounded cluster weight assumption we made.
...
However, because the cluster weights must remain within a multiplicative constant of each other by assumption, not only is this true on average, but the increase in cluster weight has to be roughly uniform across all clusters.
